Properties {
	...
	$solution_name = "$base_dir\WebApiContrib.RavenDb.sln"
	$test_dll = "$build_artifacts_dir\WebApiContrib.RavenDb.Tests.dll"
...
Task NuGetBuild -depends Clean {
	& $nuget_exe pack "$base_dir/src/WebApiContrib.RavenDb/WebApiContrib.RavenDb.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
